document.addEventListener('DOMContentLoaded', function () {
      ...
      eventRender: function(info) {
          info.el.addEventListener('contextmenu', function(e) {
              ...
              menu.querySelector('li:first-child').addEventListener('click', function() {
                  ...
                  const cateoryInput = document.getElementById('category');
  ...
  const leaveButton = document.getElementById('leave-button');
   ...
   leaveForm.addEventListener('submit', function(event) {
       ...
       // retrieve the form input values
       const holidayName = document.querySelector('#holiday-name').value;
       const leaveStartDate = document.querySelector('#leave-start-date').value;
       const leaveEndDate = document.querySelector('#leave-end-date').value;
       const leaveType = document.querySelector('#leave-type').value;

       // You can handle leave form submission here, e.g., add it to calendar or store it in localStorage
       const newLeaveEvent = {
           id: uuidv4(),
           title: holidayName,
       ...
       // Add the new leave event to the calendar
       calendar.addEvent(newLeaveEvent);

       // Save leave events to local storage
       const leaveEvents = JSON.parse(localStorage.getItem('leaveEvents')) || [];
       leaveEvents.push(newLeaveEvent);
       localStorage.setItem('leaveEvents', JSON.stringify(leaveEvents));

       leaveModal.hide();
       leaveForm.reset();
   });

   // Clear all events and leave when clear all button is clicked
   const clearAllButton = document.getElementById('clear-all-button');
   clearAllButton.addEventListener('click', function() {
       localStorage.removeItem('events'); // Remove events from local storage
       localStorage.removeItem('leaveEvents'); // Remove leave events from local storage
       calendar.removeAllEvents(); // Remove all events from the calendar
   });

  // Show leave modal when leave button is clicked
